...
<div class="container py-5">
				...
				<div class="card-body">
					<ul class="list-group list-group-flush">
						...
						<li class="list-group-item d-flex justify-content-between align-items-center">
							<span>
								<i class="{{ $icon }} me-2"></i>
								<span class="badge {{ $badgeClass }} me-2" @if($badgeStyle) style="{{ $badgeStyle }}" @endif>{{ $slug ?? 'rol' }}</span>
			...
			<div class="card h-100 shadow-sm border-0">
				...
				<div class="card-body">
					...
					<div>
						...
						<div class="d-flex flex-wrap gap-2">
							@foreach(($areas ?? []) as $area)
								<a class="btn {{ $area['btn'] }}" href="{{ route($area['name']) }}">{{ $area['label'] }}</a>
							@endforeach
							@can('adminarea')
								@if(\Illuminate\Support\Facades\Route::has('admin.profapps.index'))
									<a class="btn btn-outline-warning position-relative" href="{{ route('admin.profapps.index') }}">
										Solicitudes Pro
										@if(($totals['prof_pending'] ?? 0) > 0)
											<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill text-bg-danger">{{ $totals['prof_pending'] }}</span>
										@endif
									</a>
								@endif
							@endcan
					...
					<div>
						...
						<div class="d-flex flex-wrap gap-2">
							@php($__allPerms = \Spatie\Permission\Models\Permission::orderBy('name')->get(['name']))
							@foreach($__allPerms as $__p)
								@can($__p->name)
									<span class="badge bg-light text-dark" title="{{ $__p->name }}">{{ $__p->name }}</span>
								@endcan
							@endforeach
						</div>
					</div>
	...
	<div class="mt-4 d-flex gap-2">
		<a class="btn btn-brand" href="/">Ir al inicio</a>
		<a class="btn btn-outline-primary" href="{{ route('admin.users') }}">Usuarios</a>
		<a class="btn btn-outline-primary" href="{{ route('admin.roles.index') }}">Roles</a>
		<a class="btn btn-outline-primary" href="{{ route('admin.permissions.index') }}">Permisos</a>
	</div>
@endsection