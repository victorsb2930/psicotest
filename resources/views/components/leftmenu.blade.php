    ...
    <div class="card h-100 border-0 shadow-sm">
        <div class="card-body py-3">
            ...
            <ul class="nav flex-column gap-2">
                ...
                {{-- Link to Plans page: visible only to "normal" users (role 'user').
                     Assumption: role name for regular customers is 'user'.
                     If your project uses numeric role ids (2/3) or a different role name,
                     we can change this condition accordingly. --}}
                ...
                @php
                    $pendingFriends = 0;
                    try {
                        if (auth()->check() && Schema::hasTable('friend_requests')) {
                            $pendingFriends = \App\Models\FriendRequest::where('to_id', auth()->id())->where('status','pending')->count();
                        }
                    } catch(\Throwable $_) { $pendingFriends = 0; }
                @endphp
                <li class="nav-item position-relative">
