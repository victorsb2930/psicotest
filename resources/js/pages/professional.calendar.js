	...
	try {
		...
		const calendar = new Calendar($calendarEl[0], {
			...
			eventClick: function(info) {
				...
				if (status === 'pending') {
					if (acceptUrl) buttons.push({ text: 'Aceptar', className: 'btn-success', onClick: () => {
						axios.post(acceptUrl).then(()=>{
							window.modalNotification?.('Aceptada','Has aceptado la cita',{template:'success'});
							calendar.refetchEvents();
							closeAllModals();
						}).catch(()=>{ window.modalNotification?.('Error','No se pudo aceptar la cita',{template:'danger'}); });
					} });
					if (rejectUrl) buttons.push({ text: 'Rechazar', className: 'btn-danger', onClick: () => {
						...
						window.modalConfirm({ title: 'Confirmar rechazo', body: bodyReason, closeClick: false, buttons: [
							...
							{ text: 'Confirmar rechazo', className: 'btn-danger', onClick: (btnEl)=>{
								...
								if (reason.length < 3) {
									window.modalNotification?.('Motivo requerido','Indica al menos 3 caracteres',{template:'warning'});
									if (btnEl) btnEl.disabled = false;
									return;
								}
								axios.post(rejectUrl, { reason }).then(()=>{
									window.modalNotification?.('Rechazada','Has rechazado la cita',{template:'info'});
									calendar.refetchEvents();
									closeAllModals();
								}).catch(()=>{
									window.modalNotification?.('Error','No se pudo rechazar la cita',{template:'danger'});
									if (btnEl) btnEl.disabled = false;
								});
							}, closeOnClick: false }
						] });
				...
				if (typeof window.modalConfirm === 'function') {
					window.modalConfirm({ title: 'Detalle de cita', body, closeClick: false, buttons });
				} else {
